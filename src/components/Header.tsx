...
export function Header({ activePage, navigate }: HeaderProps) {
  ...
  const navigateAndCloseMenu = (page: Page) => {
    navigate(page);
    setIsMenuOpen(false);
  };
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-white/60 bg-white/70 backdrop-blur-xl">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8" aria-label="Main navigation">
        <button className="focus-ring flex items-center gap-3 rounded-2xl" onClick={() => navigateAndCloseMenu('landing')} aria-label="EduVerse AI home">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <button
              ...
              onClick={() => navigateAndCloseMenu(item.page)}
            >
              {item.label}
            </button>
          ))}
        </div>
        <Button onClick={() => navigateAndCloseMenu('signup')} className="hidden sm:inline-flex">Get started</Button>
        <button
          className="focus-ring grid size-12 place-items-center rounded-2xl text-ink hover:bg-white md:hidden"
          type="button"
      ...
      {isMenuOpen && (
        <div id="mobile-navigation" className="border-t border-white/60 bg-white/95 px-4 py-4 shadow-lg backdrop-blur-xl md:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-2">
            ...
            <Button fullWidth onClick={() => navigateAndCloseMenu('signup')}>Get started</Button>
          </div>
        </div>
      )}
    </header>
